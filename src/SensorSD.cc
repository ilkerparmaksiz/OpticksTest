...
namespace nexus {
  ...
  G4bool SensorSD::ProcessHits(G4Step* step, G4TouchableHistory*)
  {
    ...
    const G4VTouchable* touchable =
      ...
      //std::cout <<"Photon Here "  << touchable->GetSolid()->GetName()<<std::endl;
  ...
  void SensorSD::AddOpticksHits() {
      SEvt * sevtGPU=SEvt::Get_EGPU();
      int NumberofHits=sevtGPU->GetNumHit(0);
      for (int i=0;i<NumberofHits;i++){
          sphoton sphotonHit;
          sevtGPU->getHit(sphotonHit,i);
          G4ThreeVector position = G4ThreeVector (sphotonHit.pos.x,sphotonHit.pos.y,sphotonHit.pos.z);
          //G4ThreeVector direction = G4ThreeVector (sphotonHit.mom.x,sphotonHit.mom.y,sphotonHit.mom.z);
          //G4ThreeVector polarization = G4ThreeVector (sphotonHit.pol.x,sphotonHit.pol.y,sphotonHit.pol.z);
         std::cout <<"Sensor Hits " << position[0] <<position[1] <<position[2] <<std::endl;
